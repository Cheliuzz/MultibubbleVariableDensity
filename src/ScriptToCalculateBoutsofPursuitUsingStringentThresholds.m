...
% Set up figure
figure(9); clf
set(gcf, 'Name', 'Figure_Simplified_Ethogram');
hold on
...
for chamber = 1:size(fly_IDs,1)
    ...
    dist_all = zeros(n_targets, length(timestamps));
    bin_all = false(n_targets, length(timestamps));

    for f = 1:n_targets
        f_id = female_ids(f);
        dist_all(f,:) = arrayfun(@(i) pdist([
            trx(male_id).x_mm(i), trx(male_id).y_mm(i);
            trx(f_id).x_mm(i),    trx(f_id).y_mm(i)
        ]), 1:length(timestamps));
    ...
    % Closest female per frame logic
    target_per_frame = nan(1, length(timestamps));
    ...
    % Plot number of bouts per bin
    subplot(size(fly_IDs,1),1,chamber); hold on
    colors = lines(n_bins);  % 3 distinguishable colors

for bin = 1:n_bins
    bin_start = bin_edges(bin);
    bin_end   = bin_edges(bin+1);
    
    frame_idx = find(timestamps >= bin_start & timestamps < bin_end);
